...
将该图片中的内容归纳为markdown笔记，且不要使用小标题，小标题用数字清单代替，且所有数学符号都用mathtype表示，回答的时候不要在$前后加`符号，且在每个mathtype表达前后加空格，但mathtype表达中不要加入空格，可适当补充，令笔记完整，但笔记内容要简洁，且不要插入链接
...
2. **尾数**：尾数影响数值的精度，尾数的位数越多，精度就越高（原码）。在存储时，通常存的是 **有效数字的小数部分的二进制表示**，也称为 **有效位**
	==对于二进制规格化数，这个最高位的 `1` 会被省略（隐含存储）==，以节省存储空间，也就是说，**尾数存的只是小数点后面的部分**，小数前默认是 $1$
    * 在一种格式中（双精度），规定小数点位置在52位之前
    * 在另一种格式中（单精度），规定小数点位置在23位之前
    * <font color="#c00000">注意：当尾数用补码表示时，小数点前一位表示的是符号位，小数点后一位表示小数点前的 1，故后一位应该与符号位不同（0.1xx$表示正数，1.0xx 表示负数）</font>，不能理解

3. **阶码**：阶码反映了小数点的实际位置，即数值的缩放因子。它通常用 **移码** 来表示，即真值加上一个固定的偏移量
    * `1000 0000` 表示为 ，特殊情况 `0000 0000` 也是 $-126$
...
$$
\text{值} = (-1)^{符号位} \times (1 + 尾数) \times 2^{(阶数)}
$$
...
1. 对阶
	比较两个浮点数的阶码（即指数部分），找出谁的阶码大、谁的小
	为了让它们的阶码相同，要==把阶码小的那个数的尾数右移，同时阶码加 1==，直到两个阶码一致（小阶码向大阶码看齐）
	$A = 1.0101 × 2^4$，$B = 1.1110 × 2^2 = 0.011110 × 2^4$（阶码小的尾数右移的低位不能丢弃，保留并参加尾部运算，以确保精度，==不使用大阶码向小阶码看齐为了避免，左移高位溢出==）
	
2. 尾数加减：直接进行尾数加减
	
3. 尾数规格化
	尾数规格化就是在浮点数加减完后，把结果的尾数重新调整成符合规范的形式（通常要求尾数的最高位是非零位）
	* 右规：结果为 $\pm 1x.x\ldots x$，说明小数点往左移一位，尾数要右移一位，阶码要加 $1$，==仅需一次==
	* 左规：结果为 $\pm 0.0\ldots 01$，小数点要往右移，尾数每左移一位，阶码要减 $1$，直到尾数恢复到 $\pm 1.x\ldots x$，需要多次
	  
4. 舍入（浮点数的概念，定点数没有）
	舍入就是把计算得到的 **多余有效位** 去掉，只保留规定精度的有效数字，同时尽量**保证数值误差最小**（除了右规，对阶的时候也可能舍入）
	浮点运算中常见几种舍入方式：
	
	* 向零舍入（截断）：直接去掉超出位，不进位
	   例：3.1419 → 3.141（去掉9）
	* 向上舍入（正向舍入）：结果总是向较大的方向取
	   例：3.1412 → 3.142；−3.1412 → −3.141
	* 向下舍入（负向舍入）：结果总是向较小的方向取
	   例：3.1419 → 3.141；−3.1412 → −3.142
	* 就近舍入（最常用）：按四舍五入原则，距离最近的可表示数
	   例：3.1416 → 3.142
...
2.  现代计算机通常按字节编址，即每个字节都有一个唯一的内存地址。然而，==CPU访问内存并非逐个字节进行，而是以字长为单位（例如，32 位CPU的字长为 4 字节）==。
    ...
    * **边界对齐**：让数据的存放地址与其自身的大小对齐（==不会对填充的空位补齐==，且==变量的起始地址必须是变量本身大小的倍数==）
...
2. **存储芯片的内部结构**
    ...
    * 芯片通过一系列引脚与外部交互，主要包括：
        * **地址线**：用于接收 CPU 发来的地址信息，以定位存储单元，==可通过地址复用技术将引脚减半==
        * **数据线**：用于在芯片和 CPU 之间双向传输数据，==每一位一个引脚==
...
电容每次放电后需要重新充电（存取周期中的恢复时间），故栅极电容运行速度慢（需要刷新），但结构简单，成本低，集成度高（不复杂）
刷新的实现为对存储单元进行“读但不输出数据”，极为假读
...
2. 电容中的电荷一般只能存储 $2ms$，所以每 $2ms$ 要给所有电容重新充电（刷新）
	...
	* 集中刷新：用一段时间专门用来刷新，且该时间无法访问存储器，称为访问==死区==
	* 分散刷新：增加存取周期，用原来的时间读写，新增时间用于刷新，==优点没有死区==
	* 异步刷新：假设刷新整个芯片需要 $n$ 次，将 $n$ 次刷新均匀的分散在整个 $2ms$ 时间内，平均每段时间刷新一次，且可以在译码阶段刷新（最优，但依然存在死区）

3. DRAM 和 SDRAM 的行缓存都是采用 SRAM
	DRAM 的行缓冲是其一行的大小
...
2. 多体并行存储器
	...
	下图每一个 M 可视为一个内存条（存储体），==低位地址为片选线地址==
...
3. 单体多字存储体
	每个存储单元存储 $m$ 个字，总线宽度也为 $m$ 个字，一次并行读出 $m$ 个字，==提高读取速度==
	每次只能同时取 $m$ 个字，不能单独取其中某个字（一次读一行）
	但如果数据不在一行，需要分多次读取
...
2. 字扩展（增加芯片数量并用片选信号控制不同芯片）
	CPU 访问存储单元时，需先选则存储芯片，进行片选（片选之后选择具体的存储单元，进行读写，即为字选）
	
	片选：使用地址线的高位
	字选：使用地址线的低位
	
	* 线选法：用地址总线中未参与片内寻址的某些地址线直接作为各个芯片的片选信号
		这种方法简单，但会造成地址空间的不连续和浪费，每有一块芯片就需要一根额外的地址线
		选择某个芯片就让该芯片对应的地址线为低电平“0”，其余都为“1”
...
3. 直接映射
	...
	* 主存地址 = 标记 + 行号 + 块内地址
		标记位数 = 主存地址总位数 − 偏移量（块能容纳的字节数）− 组数的位数
	* 优缺点：
...
指令寻址：确定下一条要执行的指令的存放地址，由程序计数器 PC 来完成
	（PC 会存放下一条指令的地址，==当一条指令被取出后==，PC 的值会自动更新以指向再下一条指令）

指令寻址分为两种
1. 顺序寻址
    在顺序执行程序时，指令通常是按顺序存放在内存中的
    每次取指令结束后，PC 的值会自动增加一个==指令字的长度==（并不是 $1$），指向下一条指令的地址， $(PC) + 1 \rightarrow PC$ 
...
指令执行阶段：每条指令的执行都分为“取指令”和“执行指令”两个阶段
    在取指令阶段确定指令地址，然后获取指令；在执行指令阶段完成指令规定的操作


### 4.2.2 数据寻址

数据选址：确定本条指令的地址码指明的真实地址（并不是说地址是假的，只是寻址的方式不同，找到的地址也不同）

地址码由寻址特征（确定数据寻址方式）与形式地址（通常由 $A$ 指代）组成，每次读取指令是根据寻址特征确定寻址方式

<div style="display: flex; align-items: center;">
	<img src="assets/数据寻址寻址特征.png" style="zoom: 100%; height: 300px; margin-left: 140px;">
</div> 
...
1. 直接寻址
	...
	取指令访问内存 $1$ 次，执行指令访问内存 $1$ 次，共访问内存 $2$ 次
...
2. 间接寻址
	...
	取指令访问内存 $1$ 次，执行指令访问内存 $2$ 次，共访问内存 $3$ 次

3. 寄存器寻址
	在指令字中直接给出操作数所在的寄存器编号，即 $EA=R_i$，其操作数在由 $R_i$ 所指的寄存器内
	
	优点：指令在执行阶段不访问主存，只访问寄存器，指令字短且执行速度快，支持向量/矩阵运算
	缺点：寄存器价格昂昂贵，计算机中寄存器个数有限
	
	取指令访问内存 $1$ 次，执行指令访问内存 $0$ 次，共访问内存 $1$ 次
...
6. 立即寻址
	...
	缺点：$A$ 的位数限制了立即数的范围（如 $A$ 的位数为 $n$，且立即数采用补码时，可表示的数据范围为 $-2^{n-1}$ ~ $2^{n-1}-1$）
...
偏移寻址是指操作数的最终有效地址并不是直接在指令中给出，而是由一个“起点地址”和一个“偏移量”相加得到

有三种偏移寻址，区别在于偏移的“起点”不一样

基址寻址：以程序的起始存放地址作为“起点”
变址寻址：程序员自己决定从哪里作为“起点”
相对寻址：以程序计数器PC所指地址作为“起点”

| 寻址方式            | 有效地址 (EA) | 访存次数(指令执行期间，不算取指令) |
| :-------------- | :-------- | :----------------- |
| 相对寻址（转移指令）      | EA=(PC)+A | 1                  |
| 基址寻址（多道程序）      | EA=(BR)+A | 1                  |
...
1. 基址寻址：
	...
	注意：基址寄存器是面向操作系统的，其内容由操作系统或管理程序确定
...
2. 变址寻址
	...
	优点：数组处理过程中，可设定 $A$ 为数组的首地址，不断改变变址寄存器 IX 的内容，便可很容易形成数组中任一数据的地址，特别适合编制循环程序
	
	注意：变址寄存器是面向用户的，在程序执行过程中，变址寄存器的内容可由用户改变（作为偏移量），形式地址 $A$ 不变（作为基地址）
...
堆栈寻址是一种隐含寻址方式，它规定了操作数被存放在一个称为“堆栈”的特殊内存区域中
...
举例：基于堆栈的加法运算
* POP  ACC: $(Msp) → ACC$，$(SP)+1 → SP$ (出栈)
	把栈顶的数据取出来，放进 $ACC$ 寄存器
* POP  X: $(Msp) → X$，$(SP)+1 → SP$
	将栈顶（执行这条指令时的栈顶）的数值弹出，并保存到 $X$ 寄存器中
* ADD  Y: $(ACC)+(X) → Y$
	表示将加法的结果存放到一个名为 Y 的寄存器中
* PUSH  Y: $(SP)-1 → SP$，$(Y) → Msp$ (入栈)
	将这个数值存放到由新的 $SP$ 指针所指向的栈顶内存单元中


硬堆栈：用硬件寄存器做的堆栈，不算访存次数（优点是快，缺点是小又贵，现在基本不用了）
软堆栈：在内存里划出一块空间做的堆栈，算 $1$ 次访存次数（优点是容量大，缺点是慢一些，现在所有通用电脑都用这种）


栈顶在小地址方向 (最常见)：入栈时，地址变小。堆栈从高地址向低地址“向下”增长
栈顶在大地址方向 (比较少见)：入栈时，地址变大。堆栈从低地址向高地址“向上”增长
...
1. mov 复制指令
	$mov\ d$，$s$：将 $s$ 的内容复制到 $d$ 中
	注意：
	* 操作数大小必须一致：源操作数和目的操作数的大小（字节、字、双字等）必须相同（不能将一个 $16$ 位的寄存器 $ax$ 的内容直接传送到一个 $8$ 位的寄存器 $bl$）
    * 不能直接在内存之间传送数据：不允许两个操作数同时为内存地址，如果需要将数据从一个内存位置复制到另一个内存位置，必须通过一个寄存器作为中介
	    $mov\ ax$，$[2000H]$
	    $mov\ [si]$，$ax$
...
| 除   | divide   | $div\ s$      | 无符号数除法 $eax/s$，商存入 $eax$，余数存入 $edx$ |
|     |          | $idiv\ s$     | 有符号数除法 $eax/s$，商存入 $eax$，余数存入 $edx$ |
...
<div style="display: flex; align-items: center;">
	<img src="assets/汇编指令格式.png" style="zoom: 100%; height: 700px; margin-left: 140px;">
</div> 
...
1. jmp：无条件转移到指定地址（修改 PC 值）
	$jmp\ <地址>$：跳转到该地址
	
2. cmp：比较指令
	$cmp\ a$，$b$：比较 $a$ 和 $b$ 两个数（实际执行 $a - b$，不保存结果，仅影响标志位）
	
3. 条件跳转指令
	条件跳转指令通常与 $cmp$ 搭配使用，用于根据比较结果跳转
	
| 指令 | 英文含义 | 跳转条件 |
|------|------------|-----------|
| je <地址> | jump when equal | 若 a == b 跳转 |
| jne <地址> | jump when not equal | 若 a != b 跳转 |
| jg <地址> | jump when greater than | 若 a > b 跳转 |
| jge <地址> | jump when greater or equal | 若 a >= b 跳转 |
| jl <地址> | jump when less than | 若 a < b 跳转 |
| jle <地址> | jump when less or equal | 若 a <= b 跳转 |

4. 标号
	名字可自定义，但在使用时加 `:`（在指令中不需要使用）
	例：若 $eax > ebx$，则跳转到 $NEXT$
		$cmp\ eax，ebx$
		$jg\ NEXT$
...
loop：指令缩写
$$loop\ Looptop \xrightarrow{\text{等价于}}\begin{matrix}dec\ ecx \\
cmp\ ecx，0 \\
jne\ Looptop\end{matrix}$$

其中，$ecx$ 作为循环计数器寄存器，专门用于循环计数，不可用其他的替换
...
函数的本质是一段可被跳转执行的代码区域
它的调用和返回，本质上就是一次带参数的跳转（call）与返回（ret）
通过 ==栈== 来保存必要的信息

在计算机中，栈底一般为高地址，栈顶一般为低地址，故将栈反过来表示
每当有新的函数调用，系统会 ==向下==（低地址方向）分配新的栈帧
对帧栈内数据的访问，都是基于 $ebp$，$esp$ 进行的

<div style="display: flex; align-items: center;">
	<img src="assets/函数栈帧.png" style="zoom: 100%; height: 500px; margin-left: 140px;">
</div> 

1. 调用函数：call（函数调用）
	* call <函数名>，跳转到函数代码入口
		* 将 IP（PC在 x86 中的一种称呼）旧值压栈保存（保存在函数的栈帧顶部）  
		* 设置 IP 新值，无条件转移至被调用函数的第一条指令

2. 进入函数（建立栈帧）：push（入栈指令）
	* 将当前的栈基址寄存器（$ebp$）压栈保存
		指令：$push\ ebp$
	* 把栈顶寄存器（$esp$）的值赋给 ，建立新的函数栈基址
		指令：$mov\ ebp$，$esp$
	* 上述两个指令可以缩写为 $enter$ 指令
	* 为局部变量在栈上分配空间（通过 $sub\ esp$，$n$）

3. 函数执行阶段
	* 使用栈帧中的空间读写参数、局部变量
	* 执行具体逻辑

4. 结束函数（销毁栈帧）：ret（函数返回），pop（出栈指令）
	* 将 $ebp$ 恢复成进入函数前的值
		指令：$mov\ esp$，$ebp$
	* 从栈中弹出返回地址并跳转回调用处
		指令：$pop ebp$
	* 上述两个指令可以缩写为 $leave$ 指令
...
帧栈内部内容：
<div style="display: flex; align-items: center;">
	<img src="assets/栈帧内容.png" style="zoom: 100%; height: 500px; margin-left: 140px;">
</div> 

空闲未使用：帧栈的大小以操作系统的位数决定，必须是字长的倍数，空闲内容是栈的预留空间


调用关系：
<div style="display: flex; align-items: center;">
	<img src="assets/调用者与被调用者.png" style="zoom: 100%; height: 500px; margin-left: 140px;">
</div> 
...
# caller
caller:
push ebp                        # 保存上一层函数的基址指针（建立栈帧）
mov ebp, esp                    # 当前栈顶作为本函数的新栈帧基址
sub esp, 24                     # 为局部变量预留24字节空间

mov [ebp-12], 125               # temp1 = 125，存入局部变量区（偏移-12）
mov [ebp-8], 80                 # temp2 = 80，存入局部变量区（偏移-8）

mov eax, [ebp-8]                # eax = temp2
mov [esp+4], eax                # 把temp2作为第二个参数压入栈（参数y）

mov eax, [ebp-12]               # eax = temp1
mov esp, eax                    # 把temp1作为第一个参数压入栈（参数x）

call add                        # 调用add函数，返回值保存在eax中

mov [ebp-4], eax                # sum = eax（保存返回值）
mov eax, [ebp-4]                # eax = sum（准备返回值）
...
mov eax, [ebp+12]               # eax = 第一个参数x
mov edx, [ebp+8]                # edx = 第二个参数y
...
* 一条指令可以由一个专门的电路
* 完成有的复杂指令用纯硬件实现很困难，采用“存储程序”的设计思想，由一个比较通用的电路配合存储部件完成一条指令
* 可以理解为，提供最基础的语法和库函数，==但典型程序中 80% 的语句仅仅使用处理机中 20% 的指令（80-20规律）==
