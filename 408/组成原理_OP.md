...
1. **磁盘原理**
    * **磁盘设备的组成**
        * **存储区域**：物理上，数据存储在盘片的磁道（Track）上，磁道被划分为扇区（Sectors）。所有盘片上相同半径的磁道共同构成了柱面（Cylinders）。逻辑上，通过磁头(Heads）、柱面和扇区的编号来定位数据。
        * **硬盘存储器**：主要由磁盘驱动器、磁盘控制器和盘片组成。
        * 注意：每个盘片的正反两面都可以涂上磁性材料用来记录数据，所以磁头数一般是盘片的个数的两倍
...
1. 工作原理：将某些主存块复制到 Cache 中，缓和 CPU 与主存之间的速度矛盾（由于 Cache 的速度接近 CPU，因此可以提高程序的执行速度）
...
1. 写命中时（数据在 Cache 里）：当 CPU 对 Cache 写命中时使用
    ...
    * 写回法：只修改 Cache 的内容，而不立即写入主存，只有当此块被换出时才写回主存（减少了访问次数，但存在数据不一致的隐患）
	
2. 写未命中时（数据不在 Cache 里）：当 CPU 对 Cache 写不命中时使用
    * 写分配法：把主存中的块调入 Cache，在 Cache 中修改（==搭配写回法==）
...
指令：计算机能够识别和执行的基本命令，构成计算机程序的最小功能单位（由操作码和地址码构成）
	一台计算机的所有指令的集合构成该机的指令系统，也称为指令集（一台计算机只能执行自己指令系统中的指令）
